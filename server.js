...
app.get('/api/predictions/:stationCode', async (req, res) => {
	...
	try {
		const data = await wmataFetch(`/StationPrediction.svc/json/GetPrediction/${stationCode}`);
...
app.get('/api/elevators/:code', async (req, res) => {
	...
	try {
		const data = await wmataFetch(`/Incidents.svc/json/ElevatorIncidents?StationCode=${code}`);
		setCache(cacheKey, data);
		res.json(data);
	} catch (err) {
